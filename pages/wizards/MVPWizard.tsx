...
const steps = [
  { id: 'target', title: 'Core User', q: 'Who is the one person this MVP is built for?' },
  { id: 'job', title: 'Job to be Done', q: 'What is the singular most important task they need to achieve?' },
  { id: 'features', title: 'Must-Have Features', q: 'List 3 features without which the product is broken.' },
  { id: 'exclusions', title: 'Explicit Exclusions', q: 'What will you intentionally NOT build for V1?' },
];

export const MVPWizard: React.FC = () => {
  const [currentStep, setCurrentStep] = useState(0);
  const navigate = useNavigate();

  const next = () => {
    if (currentStep < steps.length - 1) setCurrentStep(currentStep + 1);
    else navigate('/mvp-scope');
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-6 bg-[#FDFCFB]">
      <div className="max-w-2xl w-full bg-white rounded-[60px] p-16 soft-shadow border border-gray-50 relative overflow-hidden">
        ...
        <header className="mb-12">
          <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-[0.3em] mb-2 block">Wizard — Scoping Excellence</span>
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <textarea 
              placeholder="Focus on the essentials..."
          ...
          <div className="flex items-center justify-between gap-6">
            ...
            >
              {currentStep === steps.length - 1 ? 'Finalize Scope' : 'Continue'}
            </button>
          </div>
        </div>
      </div>
